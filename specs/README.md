# Blackwake Isles Spec Index

This folder contains the canonical specs for spec-driven development.
Each spec should be updated independently, with implementation PRs linked back
to one or more spec files.

## Spec Set

- [01 Product Vision](01-product-vision.md)
- [02 Gameplay Loop and MVP](02-gameplay-loop-and-mvp.md)
- [03 Sailing and Combat](03-sailing-and-combat.md)
- [04 World Economy and Encounters](04-world-economy-and-encounters.md)
- [05 Progression UX Art Audio](05-progression-ux-art-audio.md)
- [06 Technical Architecture](06-technical-architecture.md)
- [07 Roadmap and Scope Guardrails](07-roadmap-and-scope-guardrails.md)
- [08 Stylized Ocean Rendering Upgrade](08-stylized-ocean-rendering.md)

## How To Use

- Start each implementation task by naming the spec file and section.
- Keep acceptance criteria concrete and testable.
- Treat this folder as the source of truth over older notes.
